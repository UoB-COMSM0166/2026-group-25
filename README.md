...
Environmentally, Bridge Assault is a relatively lightweight browser game compared with installed commercial games, but it is not impact-free. The playable `docs` folder is about 17.05 MB in total, including around 0.46 MB of JavaScript, 19 PNG files, and 12 MP3 files. Because the game is delivered through GitHub Pages, each page load transfers image, audio, JavaScript, and CSS assets across the network and uses energy on the player's device. The largest environmental concern is therefore repeated asset transfer, especially image files and background music, rather than server-side computation.

The game also has environmental strengths. It is served as a static site and most gameplay runs client-side, so the game does not require continuous server-side computation during play. The only remote dependency is the optional leaderboard, which sends score-related data to a PocketBase backend. The game also uses p5.js and 2D rendering rather than a heavy 3D engine, which keeps hardware requirements low and allows the game to run on ordinary laptops. This matters because software that demands newer hardware can indirectly contribute to device replacement and electronic waste.

Runtime efficiency is another environmental consideration. Bridge Assault includes many moving objects, such as enemies, bullets, coins, gems, barrels, gates, explosions, damage numbers, and UI effects. If these objects remain active unnecessarily, the game wastes CPU/GPU work. The current implementation already uses short lifetimes for temporary effects, removes collected pickups, and caps particles to reduce performance pressure. A future improvement would be to pause or reduce the p5.js draw loop when the browser tab is hidden, so the game does not keep doing unnecessary work in the background.

From an individual sustainability perspective, Bridge Assault has both benefits and risks. The simple left-right movement, automatic shooting, bilingual interface, and Level 0 tutorial reduce the learning barrier and make the game easier for new players to understand. However, the game also contains flashing effects, screen shake, fast bullets, and intense combat feedback. These effects make the game feel energetic, but they may cause discomfort for photosensitive players or players prone to migraines. A reduced-effects option would be an important accessibility improvement.

Socially, the bilingual English/Chinese UI supports inclusiveness by allowing more players in our expected audience to understand the interface. The leaderboard also creates a small competitive community by letting players compare scores. However, competition introduces moderation and fairness concerns: if fake or unrealistic scores appear, trust in the leaderboard is reduced. The backend already performs basic range validation for score and wave values, but stronger rate limiting and level-consistency checks would improve fairness further.

Privacy is another sustainability issue because leaderboard participation depends on an external service. Core gameplay works locally without remote submission, and player progress is stored in browser `localStorage`. The leaderboard only sends the fields needed for ranking, such as display name, score, wave, level, and visibility status. New leaderboard entries are private by default until the player chooses to show their score. This reduces unnecessary data exposure, although standard server infrastructure may still process request metadata such as time, IP address, or user agent. A clearer in-game privacy notice and a delete-leaderboard-data option would improve user control.

Technically, sustainability depends on whether the project can be maintained after submission. The game is organised into 26 JavaScript modules, including separate files for config, input, update logic, combat, rendering, shop, achievements, leaderboard, audio, tutorial, and persistence. This modular structure makes future changes easier because balance values, enemy data, weapon costs, and translation strings can be adjusted without rewriting the entire game loop. One technical risk is the dependency on the p5.js CDN: if the CDN is unavailable, the game may fail to load. A local fallback copy of p5.js would improve long-term availability.
