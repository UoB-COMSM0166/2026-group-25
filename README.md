...
We considered Bridge Assault using the Sustainability Awareness Framework (SusAF), which helped us look beyond direct environmental cost and consider longer-term effects across environmental, individual, social, economic, and technical dimensions. The main question was not only whether the game runs efficiently now, but what impacts would scale if more people played it regularly.

<div align="center">
<img src="docs/assets/susad_bridge_assault.svg" alt="Bridge Assault SusAD chain-of-effects diagram" width="70%" />
</div>

Environmentally, Bridge Assault is a relatively lightweight browser game compared with installed commercial games, but it is not impact-free. The playable `docs` folder is about 17.05 MB, including around 0.46 MB of JavaScript, 19 PNG files, and 12 MP3 files. Because the game is delivered through GitHub Pages, each page load transfers images, audio, JavaScript, and CSS across the network and uses energy on the player's device. The largest environmental concern is therefore repeated asset transfer, especially image and music files, rather than server-side computation.

The game also has environmental strengths. It is served as a static site and gameplay runs mostly client-side, so active play does not require continuous server-side computation. The only remote dependency is the optional leaderboard. The game uses p5.js and 2D rendering rather than a heavy 3D engine, which keeps hardware requirements low and supports use on ordinary laptops. This matters because software that demands newer hardware can indirectly encourage device replacement and electronic waste.

Runtime efficiency is another sustainability issue. Bridge Assault contains enemies, bullets, coins, gems, barrels, gates, explosions, damage numbers, and UI effects. If these remain active unnecessarily, the game wastes CPU/GPU work. The current implementation already removes collected pickups, uses short lifetimes for temporary effects, and caps particles to reduce performance pressure. A future improvement would be to pause or reduce the p5.js draw loop when the browser tab is hidden, so the game does not keep updating in the background.

Technical sustainability depends on whether the project can be maintained after submission. The game is organised into 26 JavaScript modules, including separate files for config, input, update logic, combat, rendering, shop, achievements, leaderboard, audio, tutorial, and persistence. This modular structure makes future changes easier because balance values, enemy data, weapon costs, and translation strings can be adjusted without rewriting the entire game loop. One risk is the dependency on the p5.js CDN: if the CDN is unavailable, the game may fail to load. A local fallback copy of p5.js would improve long-term availability.

Several Green Software Foundation ideas are applicable: static hosting reduces server-side computation; asset optimisation could reduce transfer size by compressing PNGs and removing unused files; visibility-aware execution could reduce background work; adaptive rendering, such as particle caps and reduced effects, can lower device workload on slower hardware; and data minimisation is partly used because leaderboard participation is optional.

#### 7.2 Ethics

The main ethical concerns are privacy, fairness, and player wellbeing. Core gameplay works locally without remote submission, and player progress is stored in browser `localStorage`. If a player joins the leaderboard, the game sends only score-related fields needed for ranking, such as display name, score, wave, level, and visibility status, to the PocketBase backend. New entries are private by default until the player chooses to show their score. The game does not intentionally collect personal identifiers beyond a user-chosen display name, although standard server infrastructure may still process request metadata such as time, IP address, or user agent. A clearer privacy notice and a delete-leaderboard-data option would improve user control.

The progression system uses in-game coins and gems only. There are no real-money purchases, loot boxes, adverts, or pressure mechanics designed to push spending. This keeps the game closer to effort-based arcade progression. The leaderboard creates useful competition, but it also needs fairness controls. The current design records score, wave, and level values, and stronger server-side validation or rate limiting would further reduce unrealistic submissions without needing invasive anti-cheat software.

#### 7.3 Accessibility

Bridge Assault includes several accessibility-supporting design choices. Automatic shooting and one-axis movement reduce the input burden compared with twin-stick shooters, while the bilingual English/Chinese interface helps more players understand menus, shop text, upgrade descriptions, and leaderboard controls. The Level 0 tutorial gives new players a low-pressure space to learn movement, gates, enemies, pickups, and boss rules before starting the main levels.

There are also limitations. The game uses flashing hit effects, screen shake, fast bullets, and intense combat feedback, which can make play exciting but may be uncomfortable for photosensitive players or players prone to migraines. Future work should add a reduced-effects option for screen shake, flashes, and particle intensity. Mobile support is also limited, so clearer device guidance or touch controls would improve access for players who primarily use phones or tablets.
